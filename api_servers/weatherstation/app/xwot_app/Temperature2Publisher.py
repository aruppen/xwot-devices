...
def handle_windows_windows_pub_POST7(request):
    ...
    logging.debug(request.requestHeaders)
    accept_type = request.requestHeaders.getRawHeaders("Accept")[0]
    lastrowid = SubscriberDB.insertClient(json_data['uri'], json_data['method'], json_data['accept'], '8')
    if not None:
        if accept_type == "application/json":
            request.setHeader("Content-Type", "application/json; charset=UTF-8")
            request.setResponseCode(200)
...
#
@app.route('/weatherstation/sensors/temperature2/pub/', branch=True)
def handle_windows_windows_pub_ws(request):
    ServerFactory = xWoTBroadcastFactory
    temperature_2 = sensors_dic['temperature_2']
    factory = ServerFactory("ws://localhost:5000/", temperature_2, debug = False,  debugCodePaths = False)
