...
#
@app.route('/window/handle/pub', methods=['GET'])
...
#
@app.route('/window/handle/pub', methods=['POST'])
...
#
@app.route('/window/handle/pub/', branch=True)
def handle_windows_windows_pub_ws2(request):
    ServerFactory = xWoTBroadcastFactory
    handle = Handle(name="Window handle")
    factory = ServerFactory("ws://localhost:5000/", handle, debug = False,  debugCodePaths = False)
...
#
@app.route('/window/handle/pub', methods=['OPTIONS'])
