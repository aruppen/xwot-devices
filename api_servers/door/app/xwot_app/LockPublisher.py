...
#
@app.route('/door/lock/pub', methods=['GET'])
...
#
@app.route('/door/lock/pub', methods=['POST'])
...
#
@app.route('/door/lock/pub/', branch=True)
def handle_windows_windows_pub_ws(request):
    ServerFactory = xWoTBroadcastFactory
    lock = Lock(name='Door lock')
    factory = ServerFactory("ws://localhost:5000/", lock, debug = False,  debugCodePaths = False)
...
#
@app.route('/door/lock/pub', methods=['OPTIONS'])
