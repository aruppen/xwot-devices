...
#
@app.route('/waterdispenser/valve/pub', methods=['GET'])
def handle_valve_pub_GET(request):
...
#
@app.route('/waterdispenser/valve/pub', methods=['POST'])
def handle_valve_pub_POST(request):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
...
#
@app.route('/waterdispenser/valve/pub/', branch=True)
def handle_valve_pub_ws(request):
    ServerFactory = xWoTBroadcastFactory
    valve = Valve(name='Valve')
    factory = ServerFactory("ws://localhost:5000/", valve, debug = False,  debugCodePaths = False)
...
#
@app.route('/waterdispenser/valve/pub', methods=['OPTIONS'])
def handle_valve_pub_OPTIONS(request):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
