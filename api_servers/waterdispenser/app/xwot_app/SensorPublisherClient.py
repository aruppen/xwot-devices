...
#
@app.route('/waterdispenser/sensor/pub/<clientid>', methods=['OPTIONS'])
def handle_sensor_pub_clientid_OPTIONS(request, clientid):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')

## GET '/windows/windows/pub/<clientid>'
#
@app.route('/waterdispenser/sensor/pub/<clientid>', methods=['GET'])
def handle_sensor_pub_clientid_GET(request, clientid):
...
#
@app.route('/waterdispenser/sensor/pub/<clientid>', methods=['PUT'])
def handle_sensor_pub_clientid_PUT(request, clientid):
    logging.debug(request.requestHeaders)
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
...
#
@app.route('/waterdispenser/sensor/pub/<clientid>', methods=['DELETE'])
def handle_sensor_pub_clientid_DELETE(request, clientid):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
    logging.debug(request.requestHeaders)
    accept_type = request.requestHeaders.getRawHeaders("Accept")[0].split(
