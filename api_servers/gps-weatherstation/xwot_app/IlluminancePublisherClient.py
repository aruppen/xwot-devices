...
#
@app.route('/weatherstation/sensors/illuminance/pub/<clientid>', methods=['OPTIONS'])
def handle_windows_windows_pub_clientid_OPTIONS4(request, clientid):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')

## GET '/windows/windows/pub/<clientid>'
#
@app.route('/weatherstation/sensors/illuminance/pub/<clientid>', methods=['GET'])
def handle_windows_windows_pub_clientid_GET4(request, clientid):
...
#
@app.route('/weatherstation/sensors/illuminance/pub/<clientid>', methods=['PUT'])
def handle_windows_windows_pub_clientid_PUT4(request, clientid):
    logging.debug(request.requestHeaders)
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
...
#
@app.route('/weatherstation/sensors/illuminance/pub/<clientid>', methods=['DELETE'])
def handle_windows_windows_pub_clientid_DELETE4(request, clientid):
    cors(request, methods=['GET', 'PUT', 'OPTIONS'])
    request.setHeader('Allow', 'GET, PUT, OPTIONS')
    logging.debug(request.requestHeaders)
    accept_type = request.requestHeaders.getRawHeaders("Accept")[0].split(
