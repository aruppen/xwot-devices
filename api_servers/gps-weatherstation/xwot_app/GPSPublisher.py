...
#
@app.route('/weatherstation/sensors/gps/pub', methods=['POST'])
def handle_windows_windows_pub_POST2(request):
    ...
    logging.debug(request.requestHeaders)
    accept_type = request.requestHeaders.getRawHeaders("Accept")[0]
    lastrowid = SubscriberDB.insertClient(json_data['uri'], json_data['method'], json_data['accept'], '3')
    if not None:
        if accept_type == "application/json":
            request.setHeader("Content-Type", "application/json; charset=UTF-8")
            request.setResponseCode(200)
...
#
@app.route('/weatherstation/sensors/gps/pub/', branch=True)
def handle_windows_windows_pub_ws2(request):
    ServerFactory = xWoTBroadcastFactory
    factory = ServerFactory("ws://localhost:5000/", gps, debug = False,  debugCodePaths = False)
...
#
@app.route('/weatherstation/sensors/gps/pub', methods=['OPTIONS'])
