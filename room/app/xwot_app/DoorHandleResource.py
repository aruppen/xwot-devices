...
# Resource:   DoorHandleResource
# Path:       /room/door/handle
#
...
class DoorHandle(XWOTService, BaseModel):

    __mutable_props__ = ['state', 'name', 'description']
    __expose__ = __mutable_props__ + ['door_link', 'room_link']

    def __init__(self):
        super(DoorHandle, self).__init__()
        self._dic = {
            'state': None,
            'name': None,
            'description': None
...
#
# GET '/room/door/handle'
#
@app.route('/room/door/handle', methods=['GET'])
def handle_room_door_handle_GET(request):
    if xwot_app.resources['door_handle']:
        accept = request.getHeader('Accept')
        d = treq.get(xwot_app.resources['door_handle'], headers={'Accept': 'application/json'})
...
#
# PUT '/room/door/handle'
#
@app.route('/room/door/handle', methods=['PUT'])
def handle_room_door_handle_PUT(request):
    if xwot_app.resources['door_handle']:
        ...
        accept = request.getHeader('Accept')
        d = treq.put(xwot_app.resources['door_handle'], data=_data, headers={'Content-Type': 'application/json'})
        request.setHeader('Content-Type', accept)
        d.addCallback(treq.content)
